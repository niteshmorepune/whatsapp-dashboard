...
export function MessageInput({
  ...
  useEffect(() => {
    if (showQuickReplies && quickReplies.length === 0) {
      axios.get("/api/quick-replies").then((r) => setQuickReplies(r.data)).catch(() => {});
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [showQuickReplies]);
  ...
  return (
    <>
      <div className="border-t border-gray-800 bg-gray-900 p-3">
        ...
        {showQuickReplies && (
          <div className="absolute bottom-20 right-4 z-50 w-72 bg-gray-800 border border-gray-700 rounded-xl shadow-xl overflow-hidden">
            <div className="px-3 py-2 border-b border-gray-700 flex items-center justify-between">
              <span className="text-xs font-medium text-gray-400">Quick Replies</span>
              <button onClick={() => setShowQuickReplies(false)} className="text-gray-500 hover:text-white">
                <X className="w-3.5 h-3.5" />
              </button>
            </div>
            {quickReplies.length === 0 ? (
              <p className="text-xs text-gray-500 text-center py-4">No quick replies yet</p>
            ) : (
              <div className="max-h-60 overflow-y-auto">
                {quickReplies.map((qr) => (
                  <button
                    key={qr.id}
                    onClick={() => {
                      setText((prev) => prev + qr.content);
                      setShowQuickReplies(false);
                      textareaRef.current?.focus();
                    }}
                    className="w-full text-left px-3 py-2.5 hover:bg-gray-700 border-b border-gray-700/50 last:border-0 transition"
                  >
                    <p className="text-xs font-medium text-white">{qr.name}</p>
